...
$subtitle = 'Perbarui Informasi Akun Petugas';
?>
...
<div class="w-full bg-slate-900/60 border border-slate-800/80 p-6 sm:p-8 rounded-3xl shadow-xl">
    ...
    <form action="<?= base_url('admin/officer/update/' . $officer['id']) ?>" method="POST" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="name" name="name" type="text" required value="<?= old('name', $officer['name']) ?>"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="email" name="email" type="email" required value="<?= old('email', $officer['email']) ?>"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="phone" name="phone" type="text" required value="<?= old('phone', $officer['phone']) ?>"
                        class="block w-full pl-10 pr-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-655 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
            ...
            <div>
                <label for="password" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Password Baru (Opsional)</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
            ...
            <div class="relative">
                ...
                <select id="bus_id" name="bus_id"
                    ...
                    <?php foreach ($buses as $bus): ?>
                        <option value="<?= $bus['id'] ?>" <?= old('bus_id', $officer['bus_id']) == $bus['id'] ? 'selected' : '' ?>>
        ...
        <div class="pt-4 flex flex-col sm:flex-row gap-3">
            <button type="submit" class="w-full sm:w-auto flex justify-center items-center py-2.5 px-6 rounded-xl font-semibold text-white bg-brand-600 hover:bg-brand-500 shadow-lg shadow-brand-600/10 gap-2 transition-all text-sm">
                <i data-lucide="check-circle" class="w-4 h-4"></i> Perbarui Petugas
            </button>
