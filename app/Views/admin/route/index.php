...
$subtitle = 'Daftar Rute Perjalanan Bus';
?>

<?= $this->section('admin_actions') ?>
<div class="flex flex-wrap items-center gap-2">
    <!-- Template Button -->
    <a href="<?= base_url('admin/route/template') ?>" class="py-2 px-3 rounded-xl font-semibold text-slate-350 bg-slate-850 hover:bg-slate-800 hover:text-white border border-slate-800 shadow flex items-center gap-1.5 transition-all text-xs">
        <i data-lucide="download" class="w-3.5 h-3.5"></i> Template CSV
    </a>
    
    <!-- Export Button -->
    <a href="<?= base_url('admin/route/export' . (!empty($search) ? '?search=' . urlencode($search) : '')) ?>" class="py-2 px-3 rounded-xl font-semibold text-slate-350 bg-slate-850 hover:bg-slate-800 hover:text-white border border-slate-800 shadow flex items-center gap-1.5 transition-all text-xs">
    ...
    <!-- Create Button -->
    <a href="<?= base_url('admin/route/create') ?>" class="py-2 px-3.5 rounded-xl font-semibold text-white bg-brand-600 hover:bg-brand-500 shadow-lg shadow-brand-600/10 flex items-center gap-1.5 transition-all text-xs">
        <i data-lucide="plus-circle" class="w-4 h-4"></i> Tambah Rute
    </a>
</div>
<?= $this->endSection() ?>
...
<div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl">
    ...
    <div class="p-5 border-b border-slate-800/80 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
            <h3 class="text-sm font-bold text-white">Daftar Rute Perjalanan</h3>
            <p class="text-xs text-slate-500 mt-0.5">Kelola rute, jarak, dan estimasi durasi perjalanan.</p>
        </div>
        
        <form action="<?= base_url('admin/route') ?>" method="GET" class="w-full md:w-80 flex gap-2">
            <div class="relative w-full">
                <input type="text" name="search" value="<?= esc($search ?? '') ?>" placeholder="Cari kota asal atau tujuan..."
            ...
            <?php if (!empty($search)): ?>
                <a href="<?= base_url('admin/route') ?>" class="py-2 px-3 rounded-xl font-semibold text-slate-400 hover:text-slate-250 bg-slate-950 border border-slate-800 text-xs transition-colors flex items-center justify-center">
                    Reset
                </a>
            <?php endif; ?>
        </form>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-800 bg-slate-900/20 text-slate-400 text-xs font-semibold uppercase tracking-wider">
                    ...
                    <th class="py-4 px-6 text-center">Jarak (KM)</th>
                    <th class="py-4 px-6 text-center">Estimasi Durasi</th>
                ...
                <?php if (empty($routes)): ?>
                    <tr>
                        <td colspan="5" class="py-8 text-center text-slate-500">
                            Belum ada data rute perjalanan. Silakan tambahkan baru atau import.
                        </td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-900/20 transition-colors">
                            ...
                            <td class="py-4 px-6 text-slate-200 font-semibold">
                                <span class="flex items-center gap-2">
                                    <i data-lucide="navigation" class="w-4 h-4 text-emerald-450"></i>
                                    <?= esc($route['destination']) ?>
                                </span>
                            </td>
                            <td class="py-4 px-6 text-center text-slate-350 font-mono"><?= esc($route['distance_km']) ?> KM</td>
                            <td class="py-4 px-6 text-center text-slate-350">
                                <?php 
                                    $hours = floor($route['estimated_duration'] / 60);
                                    $minutes = $route['estimated_duration'] % 60;
                                    echo $hours > 0 ? "{$hours} jam " : "";
                                    echo $minutes > 0 ? "{$minutes} menit" : "";
                                ?>
                            </td>
    ...
    <!-- Pagination -->
    <?php if (isset($pager)): ?>
        <?= $pager->links('routes', 'tailwind_pagination') ?>
    <?php endif; ?>
</div>
    ...
    <div class="bg-slate-900 border border-slate-800 p-6 rounded-3xl w-full max-w-md relative z-10 shadow-2xl transition-all">
        <div class="flex items-center justify-between border-b border-slate-800/80 pb-4 mb-4">
            <h3 class="text-md font-bold text-white flex items-center gap-2">
                <i data-lucide="file-input" class="w-5 h-5 text-brand-400"></i> Import Bulk Rute
            </h3>
        ...
        <form action="<?= base_url('admin/route/import') ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <div class="space-y-4">
                <p class="text-xs text-slate-400 leading-relaxed">
                    Unggah file CSV (.csv) untuk menambah banyak rute sekaligus. Pastikan struktur kolom sesuai dengan template (origin, destination, distance_km, estimated_duration).
                </p>
