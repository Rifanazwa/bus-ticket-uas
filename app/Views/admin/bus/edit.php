...
$subtitle = 'Edit Rincian Armada & Konfigurasi Kursi';
?>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="seatBuilder()">
    ...
    <div class="bg-slate-900/60 border border-slate-800/80 p-6 rounded-3xl shadow-xl lg:col-span-1 h-fit">
        ...
        <form action="<?= base_url('admin/bus/update/' . $bus['id']) ?>" method="POST" @submit="serializeLayout()">
            <?= csrf_field() ?>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input id="code" name="code" type="text" required value="<?= old('code', $bus['code']) ?>"
                        class="block w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-655 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm font-mono"
                        placeholder="EX-001">
                </div>

                <div>
                    <label for="name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nama Bus / PO</label>
                    <input id="name" name="name" type="text" required value="<?= old('name', $bus['name']) ?>"
                        class="block w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-655 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
                ...
                <div class="pt-2">
                    <button type="submit" class="w-full py-3 px-4 rounded-xl font-semibold text-white bg-brand-600 hover:bg-brand-500 shadow-lg shadow-brand-600/10 flex items-center justify-center gap-2 transition-all text-sm">
                        <i data-lucide="check-circle" class="w-4 h-4"></i> Perbarui Bus
                    </button>
...
function seatBuilder() {
    return {
        ...
        init() {
            // Load existing seat layout from database
            let loadedLayout = <?= $bus['seat_layout'] ?>;
            if (loadedLayout && loadedLayout.length > 0) {
                this.layout = loadedLayout;
                // Calculate max row number from loaded layout
                let maxRow = 1;
        ...
        toggleCell(index) {
            ...
            if (cell.type === 'seat') {
                ...
                let colLabel = String.fromCharCode(65 + seatIndexInRow);
                cell.col = colLabel;
