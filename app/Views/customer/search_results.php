...
<main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 mt-0.5 flex items-center gap-1.5">
                    ...
                    <span class="font-semibold text-slate-300"><?= count($schedules) ?> Jadwal</span>
                </p>
            </div>
        </div>
        <a href="<?= base_url('customer/home') ?>" class="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-300 border border-white/10 hover:bg-white/5 flex items-center gap-1.5 transition-all">
    ...
    <!-- RESULTS LIST -->
    <div class="space-y-3">
        <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-wider px-1 mb-4">Jadwal Tersedia</h3>
        ...
        <?php if (empty($schedules)): ?>
            <div class="glass rounded-3xl p-16 text-center space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Coba pilih tanggal atau rute yang lain.</p>
                </div>
                <a href="<?= base_url('customer/home') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-brand-600 hover:bg-brand-500 transition-all">
        ...
        <?php else: ?>
            <?php foreach ($schedules as $s): ?>
                <?php
                    ...
                    $isAIRec = $aiRecommendation && $s['id'] == $aiRecommendation['schedule_id'];
                ?>
                <div class="bus-card glass-light rounded-2xl overflow-hidden <?= $isAIRec ? 'border-brand-500/25' : '' ?>">
                    <div class="p-5 flex flex-col md:flex-row md:items-center gap-5">
                        ...
                        <!-- Price + Action -->
                        <div class="flex sm:flex-col items-center sm:items-end justify-between gap-3 sm:w-40 flex-shrink-0 border-t md:border-t-0 md:border-l border-white/5 pt-4 md:pt-0 md:pl-5">
                            ...
                            <?php if ($s['remaining_seats'] <= 0): ?>
                                <button disabled class="w-full sm:w-auto px-5 py-2.5 bg-slate-800 text-slate-600 rounded-xl text-xs font-bold border border-slate-700 cursor-not-allowed">Habis Terjual</button>
                            <?php else: ?>
                    ...
                    <div class="px-5 py-2.5 border-t border-white/5 bg-slate-950/30 flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <?php endif; ?>
                        <?php if ($isAIRec): ?>
                            <span class="text-[10px] text-brand-400 font-bold flex items-center gap-1 bg-brand-500/10 px-2 py-1 rounded-full border border-brand-500/20">
                                <i data-lucide="sparkles" class="w-3 h-3"></i> AI Recommended
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
